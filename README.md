...
<script>
"use strict";
const $ = (q, o = document) => o.querySelector(q);
const $$ = (q, o = document) => [...o.querySelectorAll(q)];
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
const byId = (id) => document.getElementById(id);
```
### 目的：建立基本工具函式，簡化 DOM 操作與非同步流程控制。

`<script>`：開始定義前端腳本區塊
   
`"use strict";`：啟用嚴格模式，避免隱性錯誤與不安全語法
    
`const $ = (q, o = document) => o.querySelector(q);`：簡化單一 DOM 元素選取，預設從 document 查詢
    
`const $$ = (q, o = document) => [...o.querySelectorAll(q)];`：簡化多個 DOM 元素選取，轉為陣列
    
`const sleep = (ms) => new Promise((r) => setTimeout(r, ms));`：建立延遲函式，回傳 Promise，延遲指定毫秒
    
`const byId = (id) => document.getElementById(id);`：簡化以 ID 取得 DOM 元素
    
### 如果要改：
    
改選取邏輯：調整 querySelector 或 querySelectorAll 的範圍
    
改延遲方式：使用 async/await 包裝 sleep
    
改命名慣例：變更 、$、byId 為更語意化名稱
    
改預設範圍：修改 o = document 為其他容器元素
    
改錯誤處理：加入 null 檢查以避免選取失敗
